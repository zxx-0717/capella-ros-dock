  ...
  wf_end_duration_{std::chrono::milliseconds(0)}
{
  ...
  wall_follow_action_server_ = rclcpp_action::create_server<capella_ros_dock_msgs::action::WallFollow>(
    node_base_interface,
  ...
  ir_intensity_sub_ = rclcpp::create_subscription<capella_ros_dock_msgs::msg::IrIntensityVector>(
    ...
    rclcpp_action::ServerGoalHandle<capella_ros_dock_msgs::action::WallFollow>> goal_handle)
{
  ...
  if (goal_handle) {
    ...
    if (goal) {
      int8_t wf_side = 0;
      if (goal->follow_side == capella_ros_dock_msgs::action::WallFollow::Goal::FOLLOW_RIGHT ||
        goal->follow_side == capella_ros_dock_msgs::action::WallFollow::Goal::FOLLOW_LEFT)
      {
        wf_side = goal->follow_side;
      } else {
        // Return error if not a valid side
        RCLCPP_WARN(logger_, "Invalid follow_side value %d", goal->follow_side);
        auto result = std::make_shared<capella_ros_dock_msgs::action::WallFollow::Result>();
      ...
      RCLCPP_INFO(
        ...
        wf_side == capella_ros_dock_msgs::action::WallFollow::Goal::FOLLOW_RIGHT ?
        "RIGHT" : "LEFT");
...
BehaviorsScheduler::optional_output_t WallFollowBehavior::execute_wall_follow(
  const std::shared_ptr<rclcpp_action::ServerGoalHandle<capella_ros_dock_msgs::action::WallFollow>>
  goal_handle,
  const RobotState & current_state)
{
  ...
  if (time_since_feedback > report_feedback_interval_) {
    // Publish feedback
    auto feedback = std::make_shared<capella_ros_dock_msgs::action::WallFollow::Feedback>();
...
void WallFollowBehavior::ir_intensity_callback(
  capella_ros_dock_msgs::msg::IrIntensityVector::ConstSharedPtr msg)
{
  ...
  const RobotState & current_state)
{
  ...
  for (const auto & hazard : current_state.hazards.detections) {
    switch (hazard.type) {
      case capella_ros_dock_msgs::msg::HazardDetection::BUMP:
      case capella_ros_dock_msgs::msg::HazardDetection::CLIFF:
      case capella_ros_dock_msgs::msg::HazardDetection::STALL:
      case capella_ros_dock_msgs::msg::HazardDetection::WHEEL_DROP:
        {
          active_hazard_frames.emplace_back(hazard.header.frame_id);
          break;
        }
      case capella_ros_dock_msgs::msg::HazardDetection::BACKUP_LIMIT:
      case capella_ros_dock_msgs::msg::HazardDetection::OBJECT_PROXIMITY:
        {
