...
#ifndef CAPELLA_ROS_DOCK__WALL_FOLLOW_STATES_HPP_
#define CAPELLA_ROS_DOCK__WALL_FOLLOW_STATES_HPP_
...
public:
  virtual bool get_next_velocity(
    ...
    const capella_ros_dock_msgs::msg::IrIntensityVector & ir_intensity,
...
public:
  ...
  : last_seen_obs_time_{start_time},
    follow_side_{follow_side},
    follow_frame_{follow_side == capella_ros_dock_msgs::action::WallFollow::Goal::FOLLOW_LEFT ?
...
public:
  ...
  : start_pose_angle_{tf2::getYaw(robot_pose.getRotation())},
    ...
    follow_side_{follow_side},
    side_ir_frames_{(follow_side == capella_ros_dock_msgs::action::WallFollow::Goal::FOLLOW_LEFT) ?
      std::vector<std::string>({"ir_intensity_side_left"}) :
      std::vector<std::string>({"ir_intensity_right"})},
    front_side_ir_frames_{(follow_side ==
      capella_ros_dock_msgs::action::WallFollow::Goal::FOLLOW_LEFT) ?
      std::vector<std::string>({"ir_intensity_front_left", "ir_intensity_left"}) :
      std::vector<std::string>({"ir_intensity_front_right"})},
    opposite_ir_frames_{(follow_side == capella_ros_dock_msgs::action::WallFollow::Goal::FOLLOW_LEFT) ?
      std::vector<std::string>({"ir_intensity_front_right"}) :
      std::vector<std::string>({"ir_intensity_front_left", "ir_intensity_left"})}
  {}
...
#endif  // CAPELLA_ROS_DOCK__WALL_FOLLOW_STATES_HPP_
