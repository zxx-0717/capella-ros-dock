...
#include "capella_ros_dock/drive_goal_behaviors.hpp"
...
: DriveGoalBaseBehavior<capella_ros_dock_msgs::action::DriveArc>(node_base_interface,
...
void DriveArcBehavior::initialize_goal(const capella_ros_dock_msgs::action::DriveArc::Goal & goal)
{
  first_iter_ = true;
  ...
  if (goal.translate_direction == capella_ros_dock_msgs::action::DriveArc::Goal::TRANSLATE_BACKWARD) {
...
std::shared_ptr<capella_ros_dock_msgs::action::DriveArc::Feedback> DriveArcBehavior::get_feedback(
  const rclcpp::Duration & time_since_feedback)
{
  if (time_since_feedback > report_feedback_interval_) {
    auto feedback = std::make_shared<capella_ros_dock_msgs::action::DriveArc::Feedback>();
    const std::lock_guard<std::mutex> lock(drive_arc_params_mutex_);
...
DriveDistanceBehavior::DriveDistanceBehavior(
  ...
  float servo_speed,
  const std::string & server_name)
:  DriveGoalBaseBehavior<capella_ros_dock_msgs::action::DriveDistance>(node_base_interface,
    node_clock_interface,
    node_logging_interface,
    node_waitables_interface,
    behavior_scheduler,
...
void DriveDistanceBehavior::initialize_goal(
  const capella_ros_dock_msgs::action::DriveDistance::Goal & goal)
{
  first_iter_ = true;
...
std::shared_ptr<capella_ros_dock_msgs::action::DriveDistance::Feedback> DriveDistanceBehavior::
get_feedback(const rclcpp::Duration & time_since_feedback)
{
  if (time_since_feedback > report_feedback_interval_) {
    auto feedback = std::make_shared<capella_ros_dock_msgs::action::DriveDistance::Feedback>();
    const std::lock_guard<std::mutex> lock(drive_distance_params_mutex_);
...
RotateAngleBehavior::RotateAngleBehavior(
  ...
  const std::string & server_name)
: DriveGoalBaseBehavior<capella_ros_dock_msgs::action::RotateAngle>(node_base_interface,
    node_clock_interface,
    node_logging_interface,
    node_waitables_interface,
    behavior_scheduler,
...
void RotateAngleBehavior::initialize_goal(
  const capella_ros_dock_msgs::action::RotateAngle::Goal & goal)
{
  first_iter_ = true;
...
std::shared_ptr<capella_ros_dock_msgs::action::RotateAngle::Feedback>
RotateAngleBehavior::get_feedback(const rclcpp::Duration & time_since_feedback)
{
  if (time_since_feedback > report_feedback_interval_) {
    auto feedback = std::make_shared<capella_ros_dock_msgs::action::RotateAngle::Feedback>();
    const std::lock_guard<std::mutex> lock(rotate_angle_params_mutex_);
    feedback->remaining_angle_travel = remain_angle_travel_;
    return feedback;
  }
...
NavigateToPositionBehavior::NavigateToPositionBehavior(
  ...
  float max_rot_speed_radps,
  const std::string & server_name)
: DriveGoalBaseBehavior<capella_ros_dock_msgs::action::NavigateToPosition>(node_base_interface,
    node_clock_interface,
    node_logging_interface,
    node_waitables_interface,
    behavior_scheduler,
...
void NavigateToPositionBehavior::initialize_goal(
  const capella_ros_dock_msgs::action::NavigateToPosition::Goal & goal)
{
  first_iter_ = true;
  ...
  BehaviorsScheduler::optional_output_t & output)
{
  ...
  if (first_iter_) {
    ...
    nav_state_.navigate_state =
      capella_ros_dock_msgs::action::NavigateToPosition::Feedback::ROTATING_TO_GOAL_POSITION;
    ...
    capella_ros_dock_msgs::action::RotateAngle::Goal rotate_goal;
  ...
  bool rotate_to_goal_heading =
    (nav_state_.navigate_state ==
    capella_ros_dock_msgs::action::NavigateToPosition::Feedback::ROTATING_TO_GOAL_ORIENTATION);
  bool drive_to_goal =
    (nav_state_.navigate_state ==
    capella_ros_dock_msgs::action::NavigateToPosition::Feedback::DRIVING_TO_GOAL_POSITION);
  if (rotate_to_position || rotate_to_goal_heading) {
    if (rotate_behavior_.iterate_on_goal(current_pose, output)) {
      if (rotate_to_position) {
        ...
        capella_ros_dock_msgs::action::DriveDistance::Goal distance_goal;
...
std::shared_ptr<capella_ros_dock_msgs::action::NavigateToPosition::Feedback> NavigateToPositionBehavior
::get_feedback(const rclcpp::Duration & time_since_feedback)
{
  const std::lock_guard<std::mutex> lock(navigate_to_position_params_mutex_);
  ...
  bool drive_to_goal =
    (nav_state_.navigate_state ==
    capella_ros_dock_msgs::action::NavigateToPosition::Feedback::DRIVING_TO_GOAL_POSITION);
  std::shared_ptr<capella_ros_dock_msgs::action::NavigateToPosition::Feedback> feedback;
  if (rotate_to_position || rotate_to_goal_heading) {
    std::shared_ptr<capella_ros_dock_msgs::action::RotateAngle::Feedback> rotate_feedback =
      rotate_behavior_.get_feedback(time_since_feedback);
    if (rotate_feedback) {
      feedback = std::make_shared<capella_ros_dock_msgs::action::NavigateToPosition::Feedback>(
        nav_state_);
      feedback->remaining_angle_travel = rotate_feedback->remaining_angle_travel;
      has_feedback = true;
    }
  } else if (drive_to_goal) {
    std::shared_ptr<capella_ros_dock_msgs::action::DriveDistance::Feedback> translate_feedback =
