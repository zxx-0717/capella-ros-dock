...
: rclcpp::Node("hazard_detection_vector_node", options)
{
  ...
  publisher_ = create_publisher<capella_ros_service_interfaces::msg::HazardDetectionVector>(
  ...
  for (std::string topic : subscription_topics_) {
    subs_vector_.push_back(
      (create_subscription<capella_ros_service_interfaces::msg::HazardDetection>(
        topic, rclcpp::SensorDataQoS(),
        [this](const capella_ros_service_interfaces::msg::HazardDetection::SharedPtr msg) {
          std::lock_guard<std::mutex> lock{this->mutex_};
